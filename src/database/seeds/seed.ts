...
async function seed() {
  ...
  try {
    ...
    // Only save if there are users to create
    let createdUsers = [];
    if (usersToCreate.length > 0) {
      ...
      // Get existing users for later use
    ...
    // Seed categories using direct SQL (since we don't have a Category entity)
    ...
    for (const user of createdUsers) {
      ...
      if (existingAddresses.length === 0) {
        ...
        // Add second address for regular users
        if (user.role !== 'admin') {
          await connection.query(`
    ...
    // Associate products with categories - use IGNORE to skip duplicates
    await connection.query(`
    ...
    // Create vouchers - use INSERT IGNORE to skip duplicates
    await connection.query(`
    ...
    // Seed orders for regular user - check if order already exists
    ...
    if (userWithRole) {
      ...
      if (userAddresses.length >= 2 && createdProducts.length >= 2) {
        ...
        if (existingOrders.length === 0) {
          ...
          // Create a second order in processing status
